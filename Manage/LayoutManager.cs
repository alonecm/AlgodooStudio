...
namespace AlgodooStudio.Manage
{
    /// <summary>
    /// 布局管理器
    /// </summary>
    internal static class LayoutManager
    {
        ...
        private static void CreateLayoutFormsFile(DockPanel panel, string name)
        {
            ...
            {
                layoutForms.RootNode.ChildNodes.Add(new Xml_Node("Form", item.DockHandler.TabText, new Xml_Attb("ID", panel.Contents.IndexOf(item).ToString())));
            }
            layoutForms.Save();
            layoutForms.Dispose();
        }
        ...
        internal static void LoadXMLFiles()
        {
             ...
             */
            //获取所有布局文件
            List<FileInfo> layoutFormsFiles = fs.GetFilesFromThisFolder("*.layoutForms");
            int index = 0;
        ...
        /// <param name="panel">停靠板</param>
        /// <param name="name">给定的布局名</param>
        internal static void EquipCurrentLayout(DockPanel panel, string name)
        {
            //判断当前布局是否为空
            if (currentLayout != null)
            {
                ...
                {
                    if (MBox.ShowWarningTips("切换布局需要关闭现有窗口，是否关闭？") == System.Windows.Forms.DialogResult.OK)
                    {
                        CloseAllForms(panel);
                        //装载前清零
                ...
                {
                    MBox.ShowInfo("请勿重复设置布局！");
                }
            }
            else
            {
                if (MBox.ShowWarningTips("切换布局需要关闭所有窗口，是否关闭？") == System.Windows.Forms.DialogResult.OK)
                {
                    CloseAllForms(panel);
                    //装载前清零
                    currentLayoutFormsIndex = 0;
